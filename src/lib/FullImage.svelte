<script lang="ts">
    ...
    let scrollY, container, dark = 0, transform = 0, clientHeight, clientWidth;

    function norm(s, f, p, os = 0, of = 1) {
        if (p < os) return s;
        if (p > of) return f;
        return s + (f - s) * cubicInOut((p - os) / (of - os));
    }

    $: if (browser) {
        let _ = scrollY, t = container?.getBoundingClientRect()?.top;
...
<div class="container" bind:this={container}>
    <div class="image"
        ...
        <div style="position: relative;width: 100%;height: 100%;">
            <div style="position: absolute;bottom: 10px;left: 10px;opacity: {1 - dark};">
                <slot/>
            </div>
        </div>
    </div>
